// tcapi/manipulation.h
// Mirrors tcapi_numpy/manipulation.py.
// Sec. C2d — tensor manipulation: set_elem, reshape, transpose, cplx_conj,
// for_each, for_each_with_coors, concatenate, stack, expand, shrink,
// extract_sub, replace_sub, real, imag, to_cplx.
#pragma once
...
// --- set_elem -----------------------------------------------------------
// Sec. C2d — write el into tensor a at the given (0-based) coordinates.
...
template<typename TenT>
void cplx_conj(const context_handle_t<TenT>& ctx, tent_t<TenT>& inout)
{
    if constexpr (std::is_same_v<elem_t<TenT>, std::complex<double>>)
...
// --- for_each -----------------------------------------------------------
// Sec. C2d — elementwise map. Three overloads:
// (1) in-place (mutating) form: f is called with a reference to each
//     element; changes it applies are written back into `inout`.
// (2) read-only (const) traversal: f is called with each element value;
//     nothing is written back (spec overload (2)).
// (3) out-of-place form (convenience): f is called with each element value;
//     the result of f is stored into `out`.
// ITensor-specific note: elements are copied out, transformed, and written
// back, since ITensor has no mutable element-proxy API.
...
template<typename TenT, typename Func>
void for_each(const context_handle_t<TenT>& ctx, tent_t<TenT>& inout, Func f)
{
    detail::ensure_active<TenT>(ctx);
...
// Read-only (const) traversal: f is invoked as f(elem) with a const element
// value; no output is produced (spec overload (2)).
template<typename TenT, typename Func>
void for_each(const context_handle_t<TenT>& ctx, const tent_t<TenT>& in, Func f)
{
    detail::ensure_active<TenT>(ctx);
    auto dims = shape<TenT>(ctx, in);
    detail::for_each_coordinate<TenT>(dims, [&](const elem_coors_t<TenT>& coors)
...
// --- for_each_with_coors ------------------------------------------------
// Sec. C2d — as for_each, but f is called with the element and its
// coordinates: f(elem, coors).
...
// Read-only (const) traversal: f is invoked as f(elem, coors) with a const
// element value; no output is produced (spec overload (2)).
template<typename TenT, typename Func>
void for_each_with_coors(const context_handle_t<TenT>& ctx,
                         const tent_t<TenT>& in,
                         Func f)
...
// --- concatenate --------------------------------------------------------
// Sec. C2d — join tensors with identical shapes along bond concat_bdidx.
// Inputs are borrowed (List<CRef<TenT>>); no copies are taken.

template<typename TenT>
tent_t<TenT> concatenate(const context_handle_t<TenT>& ctx,
...
// --- stack --------------------------------------------------------------
// Sec. C2d — stacks tensors in ins (identical shapes) along a newly
// inserted bond at index stack_bdidx; result has order r+1.

template<typename TenT>
tent_t<TenT> stack(const context_handle_t<TenT>& ctx,
                   const List<CRef<tent_t<TenT>>>& ins,
...
// --- expand -------------------------------------------------------------
// Sec. C2d — increases selected bond sizes by given increments; original
// elements preserved, appended region zero-filled.
...
// --- shrink -------------------------------------------------------------
// Sec. C2d — slices selected bonds to half-open coordinate ranges
// [coor_first, coor_second); bonds not listed keep their full range.
...
void shrink_impl(const context_handle_t<TenT>& ctx,
                 const tent_t<TenT>& in,
                 const bond_idx_elem_coor_pair_map_t<TenT>& ranges,
                 tent_t<TenT>& out)
{
    detail::ensure_active<TenT>(ctx);
    auto old_shape = shape<TenT>(ctx, in);
...
void shrink(const context_handle_t<TenT>& ctx,
            tent_t<TenT>& inout,
            const detail::bond_idx_elem_coor_pair_map_t<TenT>& bdidx_elcoor_pair_map)
{
    tent_t<TenT> out;
...
// --- extract_sub --------------------------------------------------------
// Sec. C2d — extract a subtensor given per-bond half-open coordinate
// ranges, supplied in bond-index order as a full list of (first, second)
// pairs.
...
// --- replace_sub --------------------------------------------------------
// Sec. C2d — writes sub into the target tensor at begin_pt.
...
// --- real ---------------------------------------------------------------
// Sec. C2d — elementwise real part (identity for real tensors).
template<typename TenT>
real_ten_t<TenT> real(const context_handle_t<TenT>& ctx, const tent_t<TenT>& in)
{
    detail::ensure_active<TenT>(ctx);
...
// --- imag ---------------------------------------------------------------
// Sec. C2d — elementwise imaginary part (zero for real tensors).
template<typename TenT>
real_ten_t<TenT> imag(const context_handle_t<TenT>& ctx, const tent_t<TenT>& in)
{
    detail::ensure_active<TenT>(ctx);
    auto sh = shape<TenT>(ctx, in);
    auto is_in = itensor::inds(in);
...
// --- to_cplx ------------------------------------------------------------
// Sec. C2d — real -> complex embedding (identity for complex tensors).
template<typename TenT>
cplx_ten_t<TenT> to_cplx(const context_handle_t<TenT>& ctx, const tent_t<TenT>& in)
{
    detail::ensure_active<TenT>(ctx);
    if constexpr (std::is_same_v<elem_t<TenT>, std::complex<double>>)
